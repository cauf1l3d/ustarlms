...
RESTORE_ROOT="/opt/ustar/test-env/ustar-final-dr-restore-20260823-rc"
NETWORK="ustar-final-rc-net"
POSTGRES_CONTAINER="ustar_rc_postgres"
REDIS_CONTAINER="ustar_rc_redis"
MOODLE_CONTAINER="ustar_rc_moodle"
...
sed -i 's#http://127.0.0.1:18080#http://127.0.0.1:18082#g' "${RESTORE_ROOT}/moodle/config.php"
...
test "$(curl -sS -o /dev/null -w '%{http_code}' http://127.0.0.1:18082/login/index.php)" = "200"
test "$(curl -sS -o /dev/null -w '%{http_code}' http://127.0.0.1:18082/local/ustar/data/staff_position_map_2026-08-13.csv)" = "404"
test "$(curl -sS -o /dev/null -w '%{http_code}' http://127.0.0.1:18082/local/ustar/data/structure_staffmap_2026-08-13.json)" = "404"
...
echo "isolated_release_candidate_restore=PASS"
echo "restore_root=${RESTORE_ROOT}"
echo "restore_url=http://127.0.0.1:18082"
