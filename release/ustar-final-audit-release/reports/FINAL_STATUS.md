...
- Boards CURRENT defect and isolated containment are both proven: old class reproduced 24 successes/0 conflicts; transactional row-lock candidate produced exactly 1 success/23 conflicts, final version 2. ACL, invalid JSON, >10 MiB, valid-after-rollback, 7→7 cleanup, HTTP 200, zero new critical logs and rollback→reapply all PASS.
- Workflow/Communication CURRENT audited in isolated: 70 Moodle notifications/48 unread/13 recipients but 0 USTAR notifications/providers; no official/personal-task, notification-rule, delivery or escalation tables. Synthetic notification/conversation/goal/review boundaries and exact `70/0/2/1/176` cleanup PASS. Goal API false-success for unknown actions is rejected only in isolated; old→new rollback roundtrip PASS.
- Evidence/Checklists/Gate CURRENT audited in isolated: 12 evidence mappings / 3 skills, 2 definitions / 27 checklist items / 2 runs / 10 answers, 3 routes / 9 points / one automatic `previous_adaptation` gate. Ungraded assessment, unsupported evidence labels and duplicate/stale checklist catalogs now fail closed only in the review candidate. Employee HR-publish and unassigned-submit denials, exact `12/2/10/5` cleanup, catalog hash restore and candidate→old→candidate rollback PASS.
- Organization/Reporting/Team CURRENT audited in isolated: 16 declared departments / 52 positions / reporting table present but 0 lines. Old runtime suppressed the missing-reporting warning, accepted a dangling manager ID, let HR without `viewteam` read a department payload through `position.ishead`, and included non-workforce accounts. The review candidate makes configured-state truthful, requires explicit capability, filters workforce participation and rejects dangling users. Exact `0`-row/account-type cleanup, browser warning/zero-flat-tree checks, session revocation and CURRENT→candidate rollback roundtrip PASS.
- Organization/Reporting code/audit commit `3ba63c95e43c608c94d4e9d6cbe2e975e3eb56ef` is published only to `ustar-final-audit-release`; GitHub push gate `32649280735` passed with the 128-file release manifest.
...
- Boards production deployment and TARGET policy: containment already passes `1 success / 23 conflicts` in isolated but production approval is absent. Personal/team/official types, audience/edit/share rights, immutable history, retention/transfer/archive/quota/import policy remain undecided; CURRENT must not be presented as safe TARGET collaboration.
- TARGET Tasks/Notifications/Messaging policy: separate official/personal task entities, authority/scope, notification severity/event rules, Moodle chat boundary, Bitrix/channel, acknowledgement, delivery receipt, retry/dead-letter and escalation. CURRENT Moodle projection cannot be presented as B086–B091 implementation.
- TARGET Evidence/Checklists/Gate policy: risk classes, practical/supervised evidence, mirrored adaptation checklists, reviewer authority, critical-operation scope, expiry/reminders/revocation/correction and immutable execution history. CURRENT automatic gate and overwritable checklist answers cannot be presented as admission truth.
- TARGET Organization/Reporting/Team policy: canonical staff-place/assignment source, effective periods, vacancy and temporary assignment, approval/history, direct-report versus department/named responsibility scope, CEO drill-down and interface contracts. The isolated integrity guard does not turn CURRENT departments/positions or `ishead` into TARGET.
