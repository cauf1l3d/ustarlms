...
- Workflow/Communication не реализует B086–B091: 70 Moodle alerts/48 unread/13 recipients, но 0 USTAR notifications/providers и нет official/personal task, rule, delivery или escalation tables. Goals/Reviews/HR Actions — отдельные CURRENT механизмы, не задачи. Synthetic ACL/cleanup PASS; unknown goal-action false-success contained only in isolated.
...
| Tasks/notifications semantic absence | P1 architecture / owner decision | 70 Moodle alerts but 0 USTAR providers; no Task/PersonalTask/Rule/Delivery/Escalation tables, Bitrix channel or retry/ack lifecycle | Approve B086–B091 domain model, authority/scope, event severity, channel/outbox/receipt/retry/escalation; do not rename Moodle utilities as TARGET |
