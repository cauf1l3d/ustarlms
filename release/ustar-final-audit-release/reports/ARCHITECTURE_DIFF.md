...
| B032–B038 | Route, steps, gates, admission | `route`, `route-step`, `gate`, `evidence` | Частично | 3 routes / 9 points; единственный published gate автоматически проверяет только `previous_adaptation`; reviewer/critical operation/decision/expiry/revocation отсутствуют |
| B039–B046 | Адаптация, доказательства, проверки | `checklists`, `completion`, `evidence`, `hr` | Частично/конфликт | 12 Evidence definitions используют только Moodle completion/pass-fail; 2 operational checklists не реализуют зеркальный adaptation lifecycle; immutable execution history отсутствует. Isolated fail-closed guard не заменяет TARGET policy |
...
| Tasks/notifications | B086–B091 | Isolated aggregate+ACL audit: 70/48 alerts, 13 recipients, 0 USTAR providers; no task/rule/delivery/escalation tables; only enrolment scheduler | No official/personal separation, event/severity/deadline/ack, outbox, Bitrix receipt/retry or authority scope | Approve Task/PersonalTask/NotificationRule/DeliveryAttempt/Escalation and channel/integration health model |
