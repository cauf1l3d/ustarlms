...
final class analytics {
    public static function qualification_summary(int $limit=500): array {
        ...
        $qualified=0;$withgaps=0;$unassigned=0;$unconfigured=0;$expired=0;$total=0;$gaps=[];
        foreach($users as $u){
            if(!accounts::participates((int)$u->id))continue; $total++; $pid=people::position_id((int)$u->id);
            if($pid===''||!isset($pm[$pid])){$unassigned++;continue;}
            $matrix=$st['matrix'][$pid]??[];
            // A position without requirements has no assessable standard.
            // It is neither qualified nor a demonstrated skill gap.
            if (!$matrix) {$unconfigured++;continue;}
            $ok=true;
        ...
        return ['total'=>$total,'qualified'=>$qualified,'withgaps'=>$withgaps,'unassigned'=>$unassigned,'unconfigured'=>$unconfigured,'expired'=>$expired,'coverage'=>$total?round($qualified/$total*100):0,'topgaps'=>$top,'hastopgaps'=>!empty($top)];
    }
}
