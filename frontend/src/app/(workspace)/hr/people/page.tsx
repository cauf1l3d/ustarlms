...
interface PeopleData { people:Person[]; count:number; total:number; offset:number; hasMore:boolean; positions:Position[]; departments:Dept[] }
...
export default function PeoplePage() {
  ...
  const [query,setQuery]=useState(""); const [search,setSearch]=useState(""); const [page,setPage]=useState(0); const [dept,setDept]=useState(""); const [status,setStatus]=useState("active");
  ...
  const pageSize=150;
  const load=()=>ws<PeopleData>("hr_people",{query:search,department:dept,status,limit:pageSize,offset:page*pageSize}).then(setData).catch(e=>setError(e.message));
  useEffect(()=>{
    if(!wsData?.capabilities?.hr)return;
    let active=true;
    ws<PeopleData>("hr_people",{query:search,department:dept,status,limit:pageSize,offset:page*pageSize})
      .then(result=>{if(active){setData(result);setError("")}})
      .catch(e=>{if(active)setError(e.message)});
    return ()=>{active=false};
  },[wsData,dept,status,search,page]);
  ...
  return <>
    ...
    <div className="card mb-4 p-3"><div className="grid gap-2 md:grid-cols-[1fr_220px_170px]"><label className="relative"><Search size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-mut"/><input className="input pl-10" placeholder="Имя, логин или email" value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){setPage(0);setSearch(query)}}}/></label><select className="input" value={dept} onChange={e=>{setPage(0);setDept(e.target.value)}}><option value="">Все отделы</option>{data.departments.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select><select className="input" value={status} onChange={e=>{setPage(0);setStatus(e.target.value)}}><option value="active">Активные</option><option value="suspended">Приостановленные</option><option value="all">Все</option></select></div></div>
    ...
    <nav className="mt-3 flex items-center justify-between gap-3 text-sm" aria-label="Страницы сотрудников"><span>Показано {data.count} из {data.total} · страница {page+1}</span><div className="flex gap-2"><button className="btn btn-quiet" disabled={page===0} onClick={()=>setPage(p=>p-1)}>Назад</button><button className="btn btn-quiet" disabled={!data.hasMore} onClick={()=>setPage(p=>p+1)}>Далее</button></div></nav>
  </>;
}
